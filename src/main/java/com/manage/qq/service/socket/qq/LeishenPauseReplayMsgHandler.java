...
@Component
@Slf4j
public class LeishenPauseReplayMsgHandler extends QQMsgHandler {
    @Resource
    ...
    public void handleMsg(QQMessageBO qqWsMessage) {
        ...
        if (CommandEnum.COMMAND_LEISHEN_PAUSE.judgeCommand(content)) {
            executor.submit(() -> {
                QQMsgSendRequest qqMsgSendRequest = new QQMsgSendRequest();
                qqMsgSendRequest.setMsgId(qqWsMessage.getMessageId());
                qqMsgSendRequest.setContent(LeishenUtil.pauseLeishen());
